...
namespace torasu {
	...
	// DATA
	class DataDump;
	class DataResource; //TODO DataResource
	...
	public:
		...
		virtual void setData(DataResource* data, std::map<std::string, Element*> elements) = 0;
	...
	public:
		inline ResultSegmentSettings(std::string pipeline, std::string key, ResultFormatSettings* rfs) {
	...
	private:
		...
		DataResource* data;
	public:
		inline ResultFormatSettings(std::string format, std::set<std::string>* tags, DataResource* data) {
			this->format = format;
			this->tags = tags;
			this->data = data;
		}
	...
	private:
		...
		bool jsonFormat;
	public:
		inline DataDump(unsigned char* data, int size, bool mayDelete, bool jsonFormat=false) {
			this->data = data;
			this->size = size;
